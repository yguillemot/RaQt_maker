...
for ^40 -> $i {
    # Create marbles
    @marbles[$i] = Marble.new: x => 20 + rand * 360, y=> 20 + rand * 160,
                            vx => (-1,+1).pick * rand * 0.1,
                            vy => (-1,+1).pick * rand * 0.1,
                            pen => $pen, brush => $brush;

    # Place it on the ground
    @marbles[$i].setGround: $scene;

    # and connect it to the timer
    @marbles[$i].dt = $dt;
    connect $timer, "timeout", @marbles[$i], "move";
}
